...
TcpServer::TcpServer(lane::IOManager* worker,
...
bool TcpServer::bind(lane::Address::ptr addr, bool ssl) {
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs,
    ...
    m_ssl = ssl;
    for (auto& addr : addrs) {
        // Socket::ptr sock = ssl ? SSLSocket::CreateTCP(addr) :
        // Socket::CreateTCP(addr);
        ...
        if (!sock->listen()) {
            LANE_LOG_ERROR(g_logger) << "listen fail errno=" << errno
...
void TcpServer::startAccept(Socket::ptr sock) {
    while (!m_isStop) {
        LANE_LOG_DEBUG(g_logger) << "sock->accept()";
...
void TcpServer::stop() {
    m_isStop = true;
    LANE_LOG_DEBUG(g_logger) << "m_stop = false";
...
void TcpServer::handleClient(Socket::ptr client) {
    LANE_LOG_INFO(g_logger) << "handleClient: " << *client;
}

// bool TcpServer::loadCertificates(const std::string& cert_file, const
// std::string& key_file) {
//     for(auto& i : m_socks) {
