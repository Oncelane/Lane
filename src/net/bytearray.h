...
#ifndef __LANE_BYTEARRAY_H__
#define __LANE_BYTEARRAY_H__
...
public:
     ...
     * @exception 如果getReadSize() < sizeof(uint16_t) 抛出
     * std::out_of_range
     */
     ...
     * @exception 如果getReadSize() < sizeof(uint32_t) 抛出
     * std::out_of_range
     */
     ...
     * @exception 如果getReadSize() < sizeof(uint64_t) 抛出
     * std::out_of_range
     */
     ...
     * @param[in] buf 内存缓存指针
     * @param[in] size 数据大小
     * @post m_position += size, 如果m_position > m_size 则 m_size =
     * m_position
     */
