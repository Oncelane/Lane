...
namespace lane {
    ...
    void Socket::close() {
        ...
        if (rt) {
            LANE_LOG_ERROR(g_logger)
                << "Socket::close error ::close(" << m_sock
    ...
    bool Socket::setOption(int level,
        ...
        if (rt) {
            LANE_LOG_ERROR(g_logger)
                << "Socket::setOption error ::setsockopt(" << m_sock << ","
                << level << "," << optname << "..)<errno = " << errno << ":"
                << strerror(errno) << ">";
            return false;
        }
        return true;
    }
    bool Socket::getOption(int level,
                           int optname,
                           void* optval,
                           socklen_t* optlen) {
        ...
        if (rt) {
            LANE_LOG_ERROR(g_logger)
                << "Socket::getOption error ::getsockopt(" << m_sock << ","
                << level << "," << optname << "..)<errno = " << errno << ":"
                << strerror(errno) << ">";
            return false;
        }
    ...
    bool Socket::listen(int backlog) {
        if (!isValid()) {
            LANE_LOG_ERROR(g_logger) << "listen sock = -1";
            return false;
        }
        ...
        if (rt) {
            LANE_LOG_ERROR(g_logger)
                << "Socket::listen errno ::listen(" << m_sock << "," << backlog
                << ")<errno = " << errno << ":" << strerror(errno) << ">";
            return false;
        }
        return true;
    ...
    Socket::ptr Socket::accept() {
        ...
        if (rt == -1) {
            LANE_LOG_ERROR(g_logger)
                << "Socket::accept() error ::accept(" << m_sock
                << ",...)<errno = " << errno << ":" << strerror(errno) << ">";
            return nullptr;
        }
    ...
    bool Socket::connect(Address::ptr addr, uint64_t to) {
        ...
        if (to == (uint64_t)-1) {
            ...
            if (::connect_with_timeout(
                    m_sock, addr->getAddr(), addr->getAddrLen(), to)) {
                LANE_LOG_ERROR(g_logger)
                    << "sock=" << m_sock << " connect(" << addr->toString()
                    << ") timeout=" << to << " error errno=" << errno
                    << " errstr=" << strerror(errno);
                close();
                return false;
            }
    ...
    int Socket::sendTo(const void* buffer,
        ...
        if (isConnected()) {
            return ::sendto(
                m_sock, buffer, length, flags, to->getAddr(), to->getAddrLen());
        }
        return -1;
    }
    ...
    int Socket::recvFrom(void* buffer,
        ...
        if (isConnected()) {
            ...
            return ::recvfrom(
                m_sock, buffer, length, flags, from->getAddr(), &len);
        }
        return -1;
    }
    ...
    bool Socket::cancelRead() {
        return !(
            IOManager::GetThis()->cancelEvent(m_sock, lane::IOManager::READ));
    }

    bool Socket::cancelWrite() {
        return !(
            IOManager::GetThis()->cancelEvent(m_sock, lane::IOManager::WRITE));
    }
