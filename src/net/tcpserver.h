...
#ifndef __LANE_TCP_SERVER_H__
#define __LANE_TCP_SERVER_H__
#include <functional>
#include <memory>
...
namespace lane {
    ...
    struct TcpServerConf {
        ...
        bool operator==(const TcpServerConf& oth) const {
            return address == oth.address && keepalive == oth.keepalive &&
                   ...
                   ssl == oth.ssl && cert_file == oth.cert_file &&
        ...
        TcpServerConf operator()(const std::string& v) {
            ...
            if (node["address"].IsDefined()) {
                for (size_t i = 0; i < node["address"].size(); ++i) {
                    conf.address.push_back(
                        node["address"][i].as<std::string>());
                }
            }
            return conf;
        }
    ...
    class TcpServer : public std::enable_shared_from_this<TcpServer>,
       ...
       public:
         ...
         */
        TcpServer(lane::IOManager* worker = lane::IOManager::GetThis(),
                  lane::IOManager* io_woker = lane::IOManager::GetThis(),
        ...
        // bool loadCertificates(const std::string& cert_file, const
        // std::string& key_file);
