...
namespace lane {
    ...
    bool ByteArray::isLittleEndian() const {
        return m_endian == LANE_LITTLE_ENDIAN;
    }

    void ByteArray::setIsLittleEndian(bool val) {
        if (val) {
            m_endian = LANE_LITTLE_ENDIAN;
        } else {
            m_endian = LANE_BIG_ENDIAN;
        }
    }
...
#undef XX
    ...
    bool ByteArray::writeToFile(const std::string& name) const {
        ...
        while (read_size > 0) {
            ...
            // Modify
            //  int64_t len = (read_size > (int64_t)m_baseSize ? m_baseSize :
            //  read_size) - diff;
            int64_t len = read_size > (int64_t)m_baseSize - diff
                              ? m_baseSize - diff
                              : read_size;
    ...
    uint64_t ByteArray::getWriteBuffers(std::vector<iovec>& buffers,
                                        uint64_t len) {
        if (len == 0) {
            return 0;
        }
