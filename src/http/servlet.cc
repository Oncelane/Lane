...
namespace lane {
    namespace http {
        ...
        int32_t FunctionServlet::handle(lane::http::HttpRequest::ptr request,
                                        lane::http::HttpResponse::ptr response,
                                        lane::http::HttpSession::ptr session) {
            return m_cb(request, response, session);
        }

        ServletDispatch::ServletDispatch() : Servlet("ServletDispatch") {
            m_default.reset(new NotFoundServlet("lane/1.0"));
        }

        int32_t ServletDispatch::handle(lane::http::HttpRequest::ptr request,
                                        lane::http::HttpResponse::ptr response,
                                        lane::http::HttpSession::ptr session) {
        ...
        void ServletDispatch::addGlobServletCreator(
            const std::string& uri, IServletCreator::ptr creator) {
            RWMutexType::WriteLock lock(m_mutex);
        ...
        Servlet::ptr ServletDispatch::getMatchedServlet(
        ...
        int32_t NotFoundServlet::handle(lane::http::HttpRequest::ptr request,
                                        lane::http::HttpResponse::ptr response,
                                        lane::http::HttpSession::ptr session) {
            response->setStatus(lane::http::HttpStatus::NOT_FOUND);
            response->setHeader("Server", "lane/1.0.0");
