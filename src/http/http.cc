...
void HttpRequest::init() {
    ...
    if (!conn.empty()) {
        if (strcasecmp(conn.c_str(), "keep-alive") == 0) {
            debug() << "[http] is not keep-alive";
            m_close = false;
        } else {
            debug() << "[http] is keep-alive";
            m_close = true;
        }
    }
}
...
#define PARSE_PARAM(str, m, flag, trim)                                        \
    ...
    do {                                                                       \
        ...
        m.insert(std::make_pair(                                               \
            ...
            lane::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1)))); \
        if (pos == std::string::npos) {                                        \
            break;                                                             \
        }                                                                      \
        ++pos;                                                                 \
...
void HttpRequest::initCookies() {
    ...
    PARSE_PARAM(cookie, m_cookies, ';', lane::StringUtil::Trim);
    m_parserParamFlag |= 0x4;
}
...
void HttpResponse::setCookie(const std::string& key,
    ...
    ss << key << "=" << val;
    if (expired > 0) {
        ss << ";expires=" << lane::Time2Str(expired, "%a, %d %b %Y %H:%M:%S")
           << " GMT";
    }
    if (!domain.empty()) {
        ss << ";domain=" << domain;
