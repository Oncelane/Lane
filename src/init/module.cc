...
namespace lane {
    ...
    static lane::ConfigVar<std::string>::ptr g_module_path =
        ConfigVarMgr::GetInstance()->lookUp(
            "module.path", std::string("module"), "module path");
    ...
    //                         ,sylar::Stream::ptr stream) {
    //     SYLAR_LOG_DEBUG(g_logger) << "handleNotify nty=" <<
    //     notify->toString()
    //             << " stream=" << stream;
    //     return true;
    // }
    ...
    bool Module::onConnect(lane::Stream::ptr stream) {
        return true;
    }

    bool Module::onDisconnect(lane::Stream::ptr stream) {
        return true;
    }
    ...
    //             }
    //             sd->registerServer(domain, service, ip_and_port,
    //             server_type);
    //         }
    //     }
    // }
    ...
    void ModuleManager::init() {
        ...
        lane::FSUtil::ListAllFile(files, path, ".so");
    ...
    void ModuleManager::listByType(uint32_t type,
