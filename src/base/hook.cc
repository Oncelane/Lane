...
int socket(int domain, int type, int protocol) {
    ...
    lane::FdMgr::GetInstance()->get(fd, true);
    return fd;
}
...
int connect_with_timeout(int                    fd,
    ...
    if (timeout_ms != (uint64_t)-1) {
        timer = iom->addTimer(timeout_ms, [winfo, fd, iom]() {
            ...
            t->cancelled = ETIMEDOUT;
            iom->cancelEvent(fd, lane::IOManager::WRITE);
        });
    }

    int rt = iom->addEvent(fd, lane::IOManager::WRITE);
    if (rt == 0) {
        lane::Fiber::YieldToHold();
        if (timer) {
            timer->cancel();
...
int connect(int sockfd, const struct sockaddr* addr, socklen_t addrlen) {
    return connect_with_timeout(sockfd, addr, addrlen, lane::s_connect_timeout);
}
...
ssize_t read(int fd, void* buf, size_t count) {
    return do_io(
        fd, read_f, "read", lane::IOManager::READ, SO_RCVTIMEO, buf, count);
}

ssize_t readv(int fd, const struct iovec* iov, int iovcnt) {
    return do_io(
        fd, readv_f, "readv", lane::IOManager::READ, SO_RCVTIMEO, iov, iovcnt);
}
...
ssize_t write(int fd, const void* buf, size_t count) {
    return do_io(
        fd, write_f, "write", lane::IOManager::WRITE, SO_SNDTIMEO, buf, count);
}
...
int close(int fd) {
    ...
    lane::FdCtx::ptr ctx = lane::FdMgr::GetInstance()->get(fd);
    if (ctx) {
        auto iom = lane::IOManager::GetThis();
        if (iom) {
            iom->cancelAll(fd);
        }
        lane::FdMgr::GetInstance()->del(fd);
    }
    // 不连续!!!
    return close_f(fd);
}
...
int setsockopt(int         sockfd,
    ...
    if (level == SOL_SOCKET) {
        if (optname == SO_RCVTIMEO || optname == SO_SNDTIMEO) {
            lane::FdCtx::ptr ctx = lane::FdMgr::GetInstance()->get(sockfd);
            if (ctx) {
