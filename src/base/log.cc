...
namespace lane {
    ...
    class TimeFormatItem : public LogFormatter::FormatItem {
       public:
        TimeFormatItem(const std::string& str = "%Y-%m-%d %H:%M:%S")
            : m_fmt(str) {
            if (str.empty()) {
    ...
    void LogFormatter::init() {
        ...
        while (pRt <= len) {
            if (state == 0) {
                ...
                if (pRt == len || m_pattern[pRt] == '%') {
                    if (pLt < pRt) {
                        res.push_back(std::make_tuple(
                            "s", m_pattern.substr(pLt, pRt - pLt)));
                    }
                    state = 1;  // 升级
                    pLt = pRt + 1;
                }
                ...
                // 缺少}，结尾（"\0"）默认为'}'
                if (pRt == len || m_pattern[pRt] == '}') {
                    std::get<1>(res.back()) = std::get<1>(res.back()) +
                                              m_pattern.substr(pLt, pRt - pLt);
                    state = 0;
                    pLt = pRt + 1;
                }
...
#define XX(fmt, item)                                     \
    {fmt, [](const std::string& str) -> FormatItem::ptr { \
         return FormatItem::ptr(new item(str));           \
                ...
                XX("s", StringFormatItem) XX("d", TimeFormatItem) XX(
                    "t", ThreadIdFormatItem) XX("N", ThreadNameFormatItem)
                ...
                if (node["appenders"].IsDefined()) {
                    for (int i = 0; i < (int)node["appenders"].size(); i++) {
                        ...
                        if (it["level"].IsDefined()) {
                            lad.m_level = LogLevel::FromString(
                                it["level"].as<std::string>());
                        }  // else     //默认m_level = DEBUG
