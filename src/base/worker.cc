...
namespace lane {
    ...
    static lane::ConfigVar<
        std::map<std::string, std::map<std::string, std::string> > >::ptr
        g_worker_config = lane::ConfigVarMgr::GetInstance()->lookUp(
            "workers",
    ...
    bool WorkerManager::init(
        ...
        for (auto& i : v) {
            ...
            int32_t thread_num = lane::GetParamValue(i.second, "thread_num", 1);
            int32_t worker_num = lane::GetParamValue(i.second, "worker_num", 1);
            ...
            for (int32_t x = 0; x < worker_num; ++x) {
                ...
                if (!x) {
                    ...
                    s = std::make_shared<IOManager>(
                        thread_num, name + "-" + std::to_string(x), false);
                }
                add(s);
            }
        }
