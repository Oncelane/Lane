...
namespace lane {
    ...
    Scheduler::Scheduler(uint32_t threadCount,
        ...
        if (useCur) {
            ...
            m_rootFiber.reset(
                new Fiber(std::bind(&Scheduler::run, this), true));
        ...
        // m_subQueues.resize(m_threadCount);
        // lane::WorkStealQueue<FiberAndThread> subqueue();

        // m_subQueues.emplace_back();
    }
    Scheduler::~Scheduler() {
        LANE_ASSERT(m_stop);
    ...
    void Scheduler::stop() {
        {
            ...
            if (m_rootThreadId == -1) {
                ...
                LANE_ASSERT(/*GetThis().get() == this || */ GetThis() ==
                            nullptr);
            }
    ...
    void Scheduler::idle() {
        while (!isStoped()) {
            ...
            Fiber::
                YieldToHold();  // idle协程对象会因为智能指针计数归零而自动析构
        }
    }
