...
#ifndef __LANE_CONFIG_H__

#define __LANE_CONFIG_H__
...
namespace lane {
       ...
       public:
        std::string operator()(const std::vector<T>& val) {
            ...
            for (size_t i = 0; i < val.size(); i++) {
                YAML::Node node =
                    YAML::Load(LexicalCast<T, std::string>()(val[i]));
                res.push_back(node);
            }
            ss << res;
            return ss.str();
    ...
    class ConfigVarManager {
       public:
        ...
        typedef std::unordered_map<std::string, ConfigVarBase::ptr>
            ConfigVarMapType;
        typedef RWMutex MutexType;
        ...
        typename ConfigVar<T>::ptr lookUp(const std::string& name,
            ...
            if (cv != m_cvs.end()) {
                ...
                if (res) {
                    ...
                    LANE_LOG_ERROR(LANE_LOG_NAME("system"))
                        << "ConfigVarManager::lookUp( name = " + name +
                               " , ...) exist but type not "
                        << TypeToName<T>() << " ,real type is "
                        << cv->second->getTypeName();
                    return nullptr;
                }
            }
